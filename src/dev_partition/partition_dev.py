...
if __name__ == "__main__":
    ...
    parser.add_argument("--train", type=str, default="../../data/sarc/train-balanced.csv", help="The training set to take 10% (or the first 10 items if toy) from.")
    args = parser.parse_args()

    # specfiy output filename based on --toy flag
    if args.toy:
        out_filename = "../../data/sarc/toy_train-balanced.csv"
    else:
        out_filename = "../../data/sarc/dev-balanced.csv"

    train_out_filename = "../../data/sarc/train-no-dev-balanced.csv"
    ...
    with open(args.train, "r") as train_file:
        with open(out_filename, "w") as dev_file:
            with open(train_out_filename, "w") as out_train_file:
